    ...
    exec(`${GIT} ${GIT_PROXY} config --local status.showUntrackedFiles no`)
      ...
      .then(status => {
        ...
        if (typeof status !== 'string') {
          console.log("Status not string error, trying to convert to string");
          status = status + ''; // cast status to string
        }
        const repoFiles = status
          .split('\n')  // causes error on WSL "TypeError: status.split is not a function" so we coerce status to string first
          .map(line => line.trim())
